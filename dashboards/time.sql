...
FROM mart.fact_departure GROUP BY flight_hour ORDER BY flight_hour;

SELECT EXTRACT(HOUR FROM flight_scheduled_ts) AS flight_hour, ABS(AVG(dep_delay_min)) AS earlier
FROM mart.fact_departure WHERE dep_delay_min <= 0 GROUP BY flight_hour ORDER BY flight_hour;
